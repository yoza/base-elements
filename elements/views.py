...
def details(request, page_id=None, slug=None, raise404=True,
    ...
    if request.method == 'POST' and 'formname' in request.POST and request.POST['formname'] is not None:
        response = handle_form(request.POST['formname'], request, context, request.POST['formname'])
        if response:
            return response
